classdef TransformationSystem < handle
    ...
    methods
        ...
        function [ y_arr, z_arr, dy_arr ] = rollout_w_obs_avoid( obj, y0, z0, g, input_arr, t0i, t_arr, obs1 )
            % ===========================================================================            
            % Descriptions
            % ------------
            %    [ y_arr, z_arr, dy_arr ] = rollout( obj, y0, z0, g, input_arr, t0i, t_arr, o )
            %       
            %    A full rollout of the trajectory
            %       tau * dy = z_old
            %       tau * dz = alpha_z * [ beta_z * ( g - y_old ) - z_old ] + input
            ...
            for i = 1 : Nt-1
                ...
                if t_arr( i+1 ) <= t0i
                    ...
                    % Given an obstacle, how to add a repulsive force field?
                    % calculate the distance
                    d_obs1 = obs1' - y_arr( :, i );
                    d_obs2 = [5.4, 10.5]' - y_arr( :, i );

                    dt = t_arr( i+1 ) - t_arr( i );
                    [ y_new, z_new, dy, ~ ] = obj.step( y_arr( :, i ), z_arr( :, i ), g, input_arr( :, i ) + d_obs1/norm( d_obs1 )^5 + d_obs2/norm( d_obs2 )^5, dt );
